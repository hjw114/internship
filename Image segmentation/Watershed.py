...
# Step1. 加载图像
img = cv2.imread('E:/My Documents/GitHub/pythonsy/7.png')
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # 图像灰度化

# Step2.阈值分割，将图像分为黑白两部分(转化为二值图)
...
dist_transform = cv2.distanceTransform(opening, cv2.DIST_L2, 5)  # DIST_L1 DIST_C只能对应掩膜为3； DIST_L2 可以为3或者5
...
img[markers == -1] = [0, 0, 255]   # 标注为-1 的像素点标 红 （图像分割边界）
cv2.imshow("dst", img)
cv2.waitKey(0)
