...
实时城市天气查询
目前只有主要城市
后续有空的话再实现自动查询城市代码的功能
"""
import requests
cityList_main = [   #全国主要城市代码
    #北上广深
...
city_name = input('请输入要查询的全国主要城市名称：')
for value in cityList_main:
        if value['name'] == city_name:
            code = value['code']
            r = requests.get('http://www.weather.com.cn/data/sk/'+code+'.html')
            break

r.encoding = 'utf-8'  # 解码

print(r.json()['weatherinfo']['city']+"市:\n" +
      "风向："+r.json()['weatherinfo']['WD']+"\n" +
      "实时温度："+r.json()['weatherinfo']['temp']+"\n" +
      "风力："+r.json()['weatherinfo']['WS']+"\n" +
      "湿度："+r.json()['weatherinfo']['SD']+"\n" +
      "更新时间："+r.json()['weatherinfo']['time'] )